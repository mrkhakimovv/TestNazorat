...
import { getStudents, updateStudent, deleteStudentDb, addStudent } from '../../lib/store';
import { Trash2, Edit2, Ban, CheckCircle, Search, X, UserPlus } from 'lucide-react';
            ...
            {filteredStudents.length > 0 ? (
              filteredStudents.map((student) => (
                <tr key={student.id} className={`hover:bg-slate-50 ${student.isBlocked ? 'opacity-70 bg-slate-50/50' : ''}`}>
                  <td className="px-4 py-3 font-medium text-[#1E293B]">{student.name}</td>
